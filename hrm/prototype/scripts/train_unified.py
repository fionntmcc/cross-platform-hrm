...
def compute_multistep_loss(
    ...
    for step_idx, step_logits in enumerate(all_step_logits):
        ...
        step_loss = compute_masked_loss(
            step_logits, targets, empty_mask, label_smoothing
        )
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        with torch.amp.autocast('cuda', enabled=use_amp):
            ...
            if use_multistep_loss and 'all_step_logits' in output and len(output['all_step_logits']) > 1:
                lm_loss = compute_multistep_loss(
                    output['all_step_logits'], targets, empty_mask,
                    label_smoothing=label_smoothing,
                )
            else:
...
def evaluate(
    ...
    for batch in dataloader:
        ...
        if 'all_step_logits' in output and len(output['all_step_logits']) > 1:
            ...
            lm_loss = compute_masked_loss(output['logits'], targets, empty_mask, label_smoothing=0.0)
...
def main():
    ...
    parser.add_argument('--multistep-loss', action='store_true',
