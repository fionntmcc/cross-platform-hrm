...
Usage:
    python scripts/run_model.py --model model/unified_hrm_sudoku_9x9_best.pt
    python scripts/run_model.py --model model/unified_hrm_sudoku_4x4_best.pt --puzzle sudoku_4x4
"""
...
def main():
    ...
    if data_path is None:
        data_path = PROJECT_ROOT / "data" / f"{args.puzzle}_train.npz"
    else:
        data_path = Path(data_path)

    if not data_path.exists():
        print(f"Data file not found: {data_path}")
        print("Falling back to hardcoded example puzzles.\n")
        puzzles = None
        solutions = None
    else:
    ...
    if puzzles is not None:
        ...
        # use the LAST n_eval puzzles (least likely to have been heavily trained on early)
