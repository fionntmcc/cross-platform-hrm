...
def compute_act_loss(
    ...
    """
    ponder_tensor = torch.tensor(ponder_cost, device=task_loss.device, dtype=task_loss.dtype)
...
class HaltingQTrainer:
    ...
    def update(self) -> Optional[float]:
        ...
        actions = torch.tensor(
            [t["action"] for t in batch], dtype=torch.long, device=h_H_batch.device
        )
        rewards = torch.tensor(
            [t["reward"] for t in batch], dtype=torch.float32, device=h_H_batch.device
        )
        dones = torch.tensor(
            [t["done"] for t in batch], dtype=torch.float32, device=h_H_batch.device
        )
        ...
        with torch.no_grad():
            ...
            target_q = rewards + self.discount_factor * torch.tensor(
                next_q_values, device=h_H_batch.device
            ) * (1 - dones)
