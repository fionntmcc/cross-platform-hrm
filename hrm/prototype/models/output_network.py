...
class OutputNetwork(nn.Module):
    ...
    def forward(
        ...
        if h.dim() != 2 or h.shape[-1] != self.hidden_dim:
            raise ValueError(f"Expected input shape (batch, {self.hidden_dim}), got {h.shape}")
...
def create_output_network(hidden_dim: int, grid_size: int, **kwargs) -> OutputNetwork:
    """
    ...
    """
    return OutputNetwork(hidden_dim=hidden_dim, grid_size=grid_size, **kwargs)
...
class OutputNetworkTransformer(nn.Module):
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        ...
        if self.lm_head is not None:
            ...
            raise RuntimeError(
                "No LM head available. Call tie_embedding_weights() or " "set tie_weights=False."
            )
    ...
    def predict_with_confidence(self, h: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Get predictions with confidence scores.
