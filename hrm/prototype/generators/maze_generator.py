...
def place_start_exit(maze, width, height, min_path_length, passage_cells):
    ...
    corners_edges = [
        c
        for c in passage_cells
        if c[0] <= 2 or c[0] >= height - 3 or c[1] <= 2 or c[1] >= width - 3
    ]
...
def generate_maze_dataset(
    ...
    while generated < num_mazes and attempts < max_attempts:
        ...
        metadata.append({"start_pos": start_pos, "exit_pos": exit_pos, "path_length": path_length})
    ...
    if generated < num_mazes:
        print(
            f"Warning: Only generated {generated}/{num_mazes} mazes after {max_attempts} attempts"
        )

    return {"problems": np.array(problems), "solutions": np.array(solutions), "metadata": metadata}
...
def save_dataset(dataset, filename_prefix, save_format="npz"):
    ...
    with open(f"{filename_prefix}_metadata.txt", "w") as f:
        for i, meta in enumerate(dataset["metadata"]):
            f.write(
                f"Maze {i}: start={meta['start_pos']}, exit={meta['exit_pos']}, path_length={meta['path_length']}\n"
            )
...
Visualize generated mazes with:
  python maze_visualizer.py maze_dataset.npz --num 5
    ...
    parser.add_argument(
        "-n", "--num", type=int, default=5, help="Number of mazes to generate (default: 5)"
    )
    parser.add_argument(
        "-W", "--width", type=int, default=20, help="Width of maze in cells (default: 20)"
    )
    parser.add_argument(
        "-H", "--height", type=int, default=20, help="Height of maze in cells (default: 20)"
    )
    parser.add_argument(
        "-m",
        "--min-path",
